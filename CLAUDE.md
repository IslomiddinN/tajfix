...
- **Auth** is NextAuth Credentials provider with a **JWT session strategy** (`lib/authOptions.ts`). `id` and `role` are injected into the JWT and Session via callbacks. The `Session`/`JWT`/`User` types are augmented in `types/next-auth.d.ts` to carry `id` and `role` (`'USER' | 'ADMIN' | 'MASTER' | 'SELLER'`). Passwords are bcrypt-hashed.
  ...
  - **Registration is per-app too.** The client serves `/register` (plain `USER`). The seller, master, and admin apps each serve their own **`/register`** (at the app root, outside the protected `/seller`·`/master`·`/admin` layouts), built on the shared `components/RegisterForm.tsx` → `lib/accountRegistration.ts` `createAccount()`. Each register API (`/api/seller/register`, `/api/master/register`, `/api/admin/register`) creates a **brand-new account with the right role** (plus the linked `Seller`/`Master` profile, in one transaction) — no pre-existing client account needed. The form then auto-`signIn`s and lands the user in the cabinet. Admin registration is gated by a secret `ADMIN_REGISTRATION_CODE` (env); empty ⇒ admin self-registration is off (API returns 503).
