    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-2 text-slate-600">Управление каталогом мастеров.</p>
        </div>
      ...
      {form ? (
        <div className="mb-6 rounded-[28px] border border-slate-200 bg-white p-6 shadow-card">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="text-xl font-semibold text-slate-950">{editingId ? 'Редактирование мастера' : 'Новый мастер'}</h2>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="text-sm text-slate-600">Имя<input className="adminput" {...field('name')} /></label>
            <label className="text-sm text-slate-600">Телефон<input className="adminput" {...field('phone')} /></label>
            <label className="text-sm text-slate-600">Специализация<input className="adminput" {...field('specialization')} /></label>
            <label className="text-sm text-slate-600">Цена от (сом)<input type="number" className="adminput" {...field('priceFrom')} /></label>
            <label className="text-sm text-slate-600">Рейтинг<input type="number" step="0.1" className="adminput" {...field('rating')} /></label>
            <label className="text-sm text-slate-600">Кол-во отзывов<input type="number" className="adminput" {...field('reviewsCount')} /></label>
            <label className="text-sm text-slate-600">Расстояние (км)<input type="number" step="0.1" className="adminput" {...field('distanceKm')} /></label>
            <label className="text-sm text-slate-600">Гарантия<input className="adminput" {...field('guaranteeText')} /></label>
            <label className="text-sm text-slate-600 sm:col-span-2">Ссылка на фото<input className="adminput" {...field('avatarUrl')} /></label>
            <label className="text-sm text-slate-600 sm:col-span-2">Описание<textarea className="adminput" rows={3} {...field('description')} /></label>
            <label className="flex items-center gap-2 text-sm text-slate-600"><input type="checkbox" checked={form.isAvailable} onChange={(e) => setForm((f: any) => ({ ...f, isAvailable: e.target.checked }))} /> Доступен</label>
          </div>
          ...
          <tbody className="divide-y divide-slate-100">
            {masters.map((m) => (
              <tr key={m.id}>
                ...
                <td className="px-5 py-3 text-slate-700">{m.specialization}</td>
                <td className="px-5 py-3 text-slate-700">{m.rating.toFixed(1)}</td>
                <td className="px-5 py-3">
                  <div className="flex justify-end gap-2">
                    <button onClick={() => openEdit(m)} className="rounded-full border border-slate-200 p-2 text-slate-600 hover:bg-slate-100"><Pencil className="h-4 w-4" /></button>
                    <button onClick={() => remove(m.id)} className="rounded-full border border-slate-200 p-2 text-red-600 hover:bg-red-50"><Trash2 className="h-4 w-4" /></button>
                  </div>
                </td>
              </tr>
            ))}
            {masters.length === 0 ? (
              <tr><td colSpan={4} className="px-5 py-8 text-center text-slate-500">Мастеров пока нет</td></tr>
            ) : null}
          </tbody>
        </table>
      </div>
