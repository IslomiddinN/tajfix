  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-semibold text-slate-950">Товары</h1>
          <p className="mt-2 text-slate-600">Добавление, изменение и удаление товаров магазина.</p>
        </div>
      ...
      {form ? (
        <div className="mb-6 rounded-[28px] border border-slate-200 bg-white p-6 shadow-card">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="text-xl font-semibold text-slate-950">{editingId ? 'Редактирование товара' : 'Новый товар'}</h2>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <label className="text-sm text-slate-600">Бренд<input className="adminput" {...field('brand')} /></label>
            <label className="text-sm text-slate-600">Категория
              <select className="adminput" {...field('categoryId')}>
                {categories.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
              </select>
            </label>
            <label className="text-sm text-slate-600">Цена (сом)<input type="number" className="adminput" {...field('price')} /></label>
            <label className="text-sm text-slate-600">Старая цена<input type="number" className="adminput" {...field('oldPrice')} /></label>
            <label className="text-sm text-slate-600">Скидка %<input type="number" className="adminput" {...field('discountPercent')} /></label>
            <label className="text-sm text-slate-600">Рейтинг<input type="number" step="0.1" className="adminput" {...field('rating')} /></label>
            <label className="text-sm text-slate-600">Остаток<input type="number" className="adminput" {...field('stock')} /></label>
            ...
            <label className="flex items-center gap-2 text-sm text-slate-600"><input type="checkbox" checked={form.isHit} onChange={(e) => setForm((f: any) => ({ ...f, isHit: e.target.checked }))} /> Хит</label>
            <label className="flex items-center gap-2 text-sm text-slate-600"><input type="checkbox" checked={form.isNew} onChange={(e) => setForm((f: any) => ({ ...f, isNew: e.target.checked }))} /> Новинка</label>
          </div>
          ...
          <tbody className="divide-y divide-slate-100">
            {products.map((p) => (
              <tr key={p.id}>
                ...
                <td className="px-5 py-3 text-slate-700">{p.price} сом</td>
                <td className="px-5 py-3 text-slate-700">{p.stock}</td>
                <td className="px-5 py-3">
                  <div className="flex justify-end gap-2">
                    <button onClick={() => openEdit(p)} className="rounded-full border border-slate-200 p-2 text-slate-600 hover:bg-slate-100"><Pencil className="h-4 w-4" /></button>
                    <button onClick={() => remove(p.id)} className="rounded-full border border-slate-200 p-2 text-red-600 hover:bg-red-50"><Trash2 className="h-4 w-4" /></button>
                  </div>
                </td>
              </tr>
            ))}
            {products.length === 0 ? (
              <tr><td colSpan={4} className="px-5 py-8 text-center text-slate-500">Товаров пока нет</td></tr>
            ) : null}
          </tbody>
        </table>
      </div>
