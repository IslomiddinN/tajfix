...
interface Props {
  title?: string;
  subtitle?: string;
  /** Куда вести с экрана «войдите» (для приложений с собственной страницей уведомлений). */
  loginCallback?: string;
}

/**
 * Общий список уведомлений (DB-backed /api/notifications) + переключатель push.
 * Используется клиентом и кабинетами продавца/админа — каждое приложение обслуживает
 * собственные /api/notifications и /api/push/*. Не рендерит внешний контейнер —
 * страница-обёртка сама задаёт отступы.
 */
export function NotificationsView({
  title = 'Уведомления',
  subtitle = 'Заказы, заявки, поддержка и продажи.',
  loginCallback = '/notifications'
}: Props) {
